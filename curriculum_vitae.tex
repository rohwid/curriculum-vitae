...
\cventry{Sept 2017 -- Mar 2019}{Research Assistant}{Telematics Laboratory (B. 201)}{Institut Teknologi Sepuluh Nopember}{Surabaya}{Helped the lecturers to research program and teaching activity, handled network operations and servers in the laboratory. In B201, there are a lot of servers such as Local Cloud Server with OpenStack and the others are servers with GPU to train the data for AI research purposes. The other activities, I worked with the lecturers and other researchers to build a website application (\url{http://seasinfo.com/}) to monitors the ships activities. That site was build using React as frontend and Node.js as backend, the ships data collects by IoT with AIS (Automatic identification system).}
...
\cvitem{Description}{Created program to generate the RPG player character or enemy gameplay attributes based on \textit{k}-NN and Naive Bayes algorithm then the result was classified with Neural Network Multi-class Classification. This project was created using Python and it's library.}




\section*{Computer Skills}
\cvline{Languages}{C/C++, Python, JavaScript, Node.js, Bash Shell Scripting.}

\cvline{AI Algorithms}{K-NN (Nearest Neighbor), Bayesian Method, Cluster Analysis, Regression Analysis, Classification Analysis, Decision Tree, Genetic Algorithm, Neural Network, Deep Learning.}
...
\section{Related Personal's Projects}
\cvline{\textbf{RPG Stats Generator}}{Created program to generate the RPG player character or enemy gameplay attributes based on \textit{k}-NN and Naive Bayes algorithm. This program was created using Python and it's library. (URL Link: \url{github.com/rohwid/rpg-stats-generator}).}

\cvline{\textbf{RPG Stats NN Multi- class}}{Created program to classify the RPG player character or enemy gameplay attributes that was generated before with \textit{k}-NN and Naive Bayes algorithm. The classification process uses Neural Network with Multi-class Classification. This program was created using Python with TensorFlow and other Python library. (URL Link: \url{github.com/rohwid/rpg-stats-nn-multiclass}).}

\cvline{\textbf{Emotion Recognition}}{Create the emotion detection model with VGG16. The goal was to detect 7 emotions (neutral, happy, sad, surprise, angry, fear and disgusted) but in this project only 5 emotion (neutral, happy, sad, surprise, and angry) which stable enough when detect the people emotions. This program was created using Python with OpenCV, TensorFlow, and other Python library. (URL Link: \url{github.com/rohwid/emotion-recognition}).}
